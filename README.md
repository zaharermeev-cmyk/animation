...
## Цикл
Пустой экран → от центра материализуются плитки → через 2 с появляется шахтёр → сцена с лисой → сцена тает, плитки пропадают от краёв к центру → пауза → всё заново.
...
bg.scene.setFx('finale', true); bg.restartScene();
```
| ключ | что делает |
|---|---|
| `paws` | светящиеся следы лап, постепенно гаснут |
| `schema` | «схема» на плитках: узлы, пунктирные связи, по маршрутам бегут монеты |
...
| `finale` | в конце монеты собираются в маску вора, потом всё гаснет |
