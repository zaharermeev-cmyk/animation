...
## Эффекты (тумблеры)
Всегда включены (в панели скрыты): схема на плитках, спящий охранник, курсор-фонарик.
В панели «Эффекты» (по умолчанию выключены, выбор запоминается в браузере):

| ключ | что делает |
|---|---|
| `paws` | светящиеся следы лап |
| `sIcons` | иконки в узлах схемы: карта, кошелёк, банк, телефон, сейф, ₿, глобус, ключ |
| `sPulses` | по связям бегут светящиеся импульсы, узел вспыхивает при прибытии |
| `sWave` | при краже от кучки шахтёра по полу расходится волна, схема вспыхивает |
| `sBuild` | связи чертятся по одной «пером», в финале стираются в обратном порядке |
| `sLabels` | подписи лёжа на плитках (`#A7-0x3F`, `mixer`, `→ BTC`…), время от времени меняются |
| `sCounters` | над узлами тикают суммы `$ 12 400` |
...
| `sRadar` | по полу проходит луч радара, подсвечивая узлы и связи |
| `sFoxPath` | путь бегства лисы рисуется пунктиром и пристыковывается к схеме |
| `sMiner` | от кучки шахтёра к схеме бежит импульс на каждый удар кирки |
