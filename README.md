...
```js
bg.scene = new MinerScene(bg, { fx: { paws: true, spot: true } });
bg.scene.setFx('safe', true); bg.restartScene();
```
| ключ | что делает |
|---|---|
| `paws` | светящиеся следы лап, постепенно гаснут |
| `spot` | прожектор охраны скользит по полу; лиса замирает, когда луч рядом |
| `schema` | «схема» на плитках: узлы, пунктирные связи, по маршрутам бегут монеты |
| `safe` | сейф вместо кучки: лиса крутит кодовый замок, дверца открывается |
| `sack` | лиса уносит мешок с дыркой — за ней остаётся дорожка монет |
